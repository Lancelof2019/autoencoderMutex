...
class Autoencoder(tf.keras.models.Model):
    ...
    def train(self, train_index, val_index, inputs, init,logger):
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            if last_improvement > max_requirement:
        ...
        #print("=========================finish one mini bacth in iterator:"+str(self.iterator)+"===================================")
...
def partialProcess(iterator, selected_features, inputhtseq, inputmethy, act):
   ...
   ##################################################
    ...
    if htseq_nbr > 1 and methy_nbr > 1:
        ...
        print(type(X_train1_data_orig))
        print(X_train1_data_orig.shape)
        X_train1, X_test1 = model_selection.train_test_split(X_train1_data_orig, test_size=0.2, random_state=1)
        X_train2, X_test2 = model_selection.train_test_split(X_train2_data_orig, test_size=0.2, random_state=1)



        #X_train1_var = tf.Variable(X_train1, dtype=tf.float64)
        #X_test1_var = tf.Variable(X_test1, dtype=tf.float64)

        #X_train2_var = tf.Variable(X_train2, dtype=tf.float64)
       # X_test2_var = tf.Variable(X_test2, dtype=tf.float64)
       ...
       # print(best)
        ...
        f = open(
            r'C:\Users\gklizh\Documents\Workspace\code_and_data19\data\python_related\result\comm_parameter_binary_' + str(
...
def parallel_processing(selected_features, inputhtseq, inputmethy, num_processes, act, community_num):
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        #pool.map(process_iteration, community_num)
...
if __name__ == '__main__':
   ...
   # with open(fname, 'wb+') as fpkl:
    ...
    feature_path = os.path.join(r'C:\Users\gklizh\Documents\Workspace\code_and_data19\data\python_related\data',
                         cancer_type + "_selected_features01.csv")

    selected_features = np.genfromtxt(feature_path,delimiter=',', skip_header=1)
    # log10 (fpkm + 1)

    exp_path = os.path.join(r'C:\Users\gklizh\Documents\Workspace\code_and_data19\data\python_related\data',
    ...
    mty_path = os.path.join(r'C:\Users\gklizh\Documents\Workspace\code_and_data19\data\python_related\data',
