...
class encoderNetwork(tf.keras.models.Model):
    ...
    def call(self, inputs, is_train):
        ...
        self.bias2 = self.ensmallNetwork.l1_layer.bias
...
class outputSmallNetwork(tf.keras.layers.Layer):
    def __init__(self, n_input1, n_input2, activation, _init):
        ...
        self.l5_layer = tf.keras.layers.Dense(self.n_hidden5, kernel_initializer=_init, name='layer1')
        self.l6_layer = tf.keras.layers.Dense(self.n_hidden6, kernel_initializer=_init, name='layer2')
        # self.l1=None
        # self.l2=None
        # self.layer2 = tf.keras.layers.Dense(self.n_hidden2, activation='relu')
        # tf.keras.layers.BatchNormalization()(l1, training=bool(self.is_train))
...
class Autoencoder(tf.keras.models.Model):
    ...
    def lossfun(self, sampleInput, is_train, init):
        ###########################################################################
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            for sample in mini_batches:
                ...
                with tf.GradientTape() as tape:
                    ...
                    # else:
                    current_loss = self.lossfun([batch_xs1, batch_xs2], self.is_train, init)  # 1
                ...
                logger.info(
                    "-----------------------------------------patch ends in train-----------------------------------------")
                cost = self.lossfun([batch_xs1, batch_xs2], self.is_train, init)  # 2
            ...
            for sample in mini_batches:
                ...
                cost_val = self.lossfun([batch_xs1, batch_xs2], self.is_train, init)
            ...
            # logger.info("\n")
            costs_inter.append(avg_cost)#1.1041128635406494,1.1026578,1.08701980113983155
            costs_val_inter.append(avg_cost_val)#0.7548856139183044,0.7347193956375122,0.7152150869369507
...
def train_test(params, testInput, iterator,logger):
    ...
    while epoch < sae_test.max_epochs and stop == False:
        ...
        for sample in mini_batches:
            ...
            # cost=current_loss
            cost = sae_test.lossfun([batch_xs1, batch_xs2], is_train, _init)
        ...
        if avg_cost < best_cost:
            ...
            # costs_inter = []
        # costs_inter = []
        if last_improvement > sae_test.require_improvement:
...
def partialProcess(iterator, selected_features, inputhtseq, inputmethy, act):
    ...
    fname = r'C:\Users\gklizh\Documents\Workspace\code_and_data12\data\python_related\result\comm_trials_binary_test38' + str(
        iterator) + '.pkl'
   ...
   ##################################################
    ...
    if htseq_nbr > 1 and methy_nbr > 1:
        ...
        f = open(
            r'C:\Users\gklizh\Documents\Workspace\code_and_data12\data\python_related\result\comm_parameter_binary38' + str(
                iterator) + '.txt',
            'a+')
